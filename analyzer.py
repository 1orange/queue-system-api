...
parser = argparse.ArgumentParser(description='Get priority of conditions with given time')

parser.add_argument("-t", "--time", dest="wait_time", required=True,
...
def eval_priority_by_wait_time(elapsed_minutes):
    ...
    for condition in get_all_conditions():
        eval_result[condition.name] = (int(elapsed_minutes) * 60 * condition.urgency) / condition.burst_time
